...
namespace PvPterraUtils.Handlers
{
    public class PvPTerraRegion
    {
        ...
        /// <summary>
        /// Comprueba constantemente si el jugador está en el equipo que le asignamos.
        /// Si no lo está, lo fuerza a regresar.
        /// </summary>
        private static void EnforceAssignedTeam(PvPPlayer pvpPlayer)
        {
            ...
            // Si el equipo actual del jugador no coincide con el que le dictó el plugin
            if (pvpPlayer.TSPlayer.TPlayer.team != assignedTeam)
            {
                // Le devolvemos su color correcto en la memoria
                pvpPlayer.TSPlayer.TPlayer.team = assignedTeam;

                // Le enviamos un paquete para que su pantalla se actualice al instante
                NetMessage.SendData((int)PacketTypes.PlayerTeam, -1, -1, null, pvpPlayer.TSPlayer.Index);

                // Le damos un tirón de orejas en el chat
                pvpPlayer.TSPlayer.SendErrorMessage(PvPTerrai18n.GetString("Reg_TeamRestored"));
            }
        }


        // Añadimos 'async' a la declaración del método
        private static void EnterPvPRegion(PvPPlayer pvpPlayer, string regionName)
        {
            try
            {
                ...
                // --- 2. ESCUDO DE PARTIDA EN CURSO O PREFERENCIA DE ESPECTADOR ---
                ...
                // Entra como espectador si la partida ya empezó (y no es FFA) O si el jugador lo eligió por comando
                if (isMatchActive || pvpPlayer.PreferredTeam.Equals("Spectator", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // El espectador no hace fila para pelear
                ...
                // --- 3. ASIGNACIÓN DE EQUIPO INTELIGENTE (EL PORTERO) ---
                ...
                // 4. Sincronizar el equipo pero con el PvP APAGADO
                Utils.PvPTerraMisc.SyncPvPState(tsPlayer, false, pvpPlayer.Team);

                // 5. Aplicar buffs de región (si los hay)
                ...
                // 6. Registramos la hora de entrada y llamamos al Cerebro para que lo ponga en espera
                pvpPlayer.EntryTime = DateTime.Now;
                UpdateMatchState(regionName, regionConfig);
            }
            catch (Exception ex)
            {
                // Error detallado para el administrador en la consola
                TShock.Log.ConsoleError(PvPTerrai18n.GetString("Log_EnterRegionError", ex.Message, ex.StackTrace));

                // Mensaje sencillo y regionalizado para el jugador
        ...
        {
            // --- LÓGICA DE SALIDA SEGURA DE LA ARENA ---
            ...
            // 1. Detectar si huyó cobardemente en combate
            ...
            // 2. Limpiar estados internos de PvP y apagar hostile
            pvpPlayer.ResetPvPState();
            Utils.PvPTerraMisc.SyncPvPState(tsPlayer, false, "None");

            // 3. Seguro anti-lobby y restauración de pertenencias
            // FIX: Los espectadores SIEMPRE tienen un inventario modificado, sin importar si la arena usa inventarios o no.
            ...
            // 4. Avisar al juez de la partida
            CheckMatchWinner(oldRegion);

            // 5. Limpieza estricta de buffs si aplica
        ...
        private static void EnforcePvPBuffs(PvPPlayer pvpPlayer, bool forceUpdate = false)
        {
            try
            {
                ...
                // ==========================================
                // MODO DICTADOR DE EQUIPO (Snap-back continuo)
                // ==========================================
                int assignedTeam = Utils.PvPTerraMisc.GetTeamIdFromName(pvpPlayer.Team);

                // Si su equipo en el juego es distinto al que dice el plugin, se lo cambiamos a la fuerza
                if (pvpPlayer.TSPlayer.TPlayer.team != assignedTeam)
                {
                    pvpPlayer.TSPlayer.TPlayer.team = assignedTeam;
                    NetMessage.SendData((int)PacketTypes.PlayerTeam, -1, -1, null, pvpPlayer.TSPlayer.Index);

                    // Mensaje regionalizado usando la clave de restauración de equipo
                    pvpPlayer.TSPlayer.SendErrorMessage(PvPTerrai18n.GetString("Reg_TeamRestored"));
                }
                ...
                if (regionConfig.PvPbuffs == null) return; // Escudo
                ...
                foreach (int buffId in regionConfig.PvPbuffs)
                {
                    ...
                    {
                        pvpPlayer.TSPlayer.SetBuff(buffId, 3600 * 60); // Aplica por 1 hora
                        reapplied = true;
                    }
                }
