...
namespace PvPterraUtils.utils
{
    ...
    {
        // Idioma por defecto. Podría configurarse también desde PvPTerraConfig.json
        public static string CurrentLang = "en";

        // Diccionario principal: Idioma -> (Clave -> Traducción)
        ...
        /// <summary>
        /// Obtiene la cadena de texto traducida según la clave.
        /// Soporta el uso de string.Format() interno para inyectar variables como nombres o cantidades.
        /// </summary>
        public static string GetString(string key, params object[] args)
        {
            // Buscamos en el idioma actual
            ...
            // Fallback a Español si falta la traducción en el idioma seleccionado
            if (_texts["es"].ContainsKey(key))
            {
                string text = _texts["es"][key];
                return args.Length > 0 ? string.Format(text, args) : text;
            }

            // Si la clave no existe en ningún lado, devolvemos la clave misma para notar el error
